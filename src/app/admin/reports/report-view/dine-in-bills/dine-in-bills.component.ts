...
y = data.cursor.y;
    ...
    this.downloadService.saveAndOpenFile(doc.output('datauristring'),'Dine In Bills' + new Date().toLocaleString() + '.pdf','pdf','application/pdf');
  }
  ...
  downloadExcel() {
    ...
    this.downloadService.saveAndOpenFile(
      base64Data,
      'Dine In Bills' + new Date().toLocaleString() + '.csv','csv','text/csv');
  }
